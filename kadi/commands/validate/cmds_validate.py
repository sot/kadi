...
kadi.logger.logLevel = "DEBUG"
# os.environ["KADI"] = os.path.expanduser("~/git/kadi/early")
os.environ["KADI_COMMANDS_VERSION"] = "2"
# os.environ["KADI_SCENARIO"] = "early"
...
def get_telem_values(tstart, msids, days=14, dt=32.8, name_map={}):
    ...
    out = np.rec.fromarrays(
        [msidset.times] + [msidset[x].vals for x in msids], names=outnames
    )
    return out
...
def get_bad_times() -> Table:
    url = BAD_TIMES_SHEET_URL.format(
        doc_id=conf.cmd_events_flight_id, gid=conf.cmd_events_bad_times_gid
    )
...
def validate_cmd_states(days=4, run_start_time=None, scenario=None):
    ...
    diff_only = {
        "pointing": {"diff": angle_diff * 3600, "date": tlm["date"][ok]},
        "roll": {"diff": roll_diff * 3600, "date": tlm["date"][ok]},
    }
        ...
        if msid not in diff_only:
            if msid in MODE_MSIDS:
                ...
                ticklocs, fig, ax = plot_cxctime(
                    tlm["date"], tlm[msid], fig=fig, ax=ax, fmt="-r"
                )
                ticklocs, _, _ = plot_cxctime(
                    tlm["date"], state_msid, fig=fig, ax=ax, fmt="-b"
                )
                ax.set_yticks(range(len(MODE_MSIDS[msid])), MODE_MSIDS[msid])
            else:
                ticklocs, fig, ax = plot_cxctime(
                    tlm["date"], tlm[msid] / scale, fig=fig, ax=ax, fmt="-r"
                )
                ticklocs, fig, ax = plot_cxctime(
                    tlm["date"], pred[msid] / scale, fig=fig, ax=ax, fmt="-b"
                )
        else:
            ticklocs, fig, ax = plot_cxctime(
                diff_only[msid]["date"],
...
def get_states_kadi(datestart, datestop):
    ...
    logger.info("Getting commanded states between %s - %s" % (datestart, datestop))
