...
kadi_moves = events.grating_moves.filter(
    start="2000:160:12:00:00", stop="2014:008:12:00:00", grating__contains="ETG"
).table
mta_moves = Table.read(
    "mta_grating_moves.dat",
    format="ascii",
    converters={
        "START_TIME": [ascii.convert_numpy(str)],
...
kadi_mta = Table(
    [i_nearest, kadi_moves["start"], mta_nearest["START_TIME"], dt],
...
mta_kadi = Table(
    [i_nearest, kadi_nearest["start"], mta_moves["START_TIME"], dt],
    names=["i_nearest", "kadi_date", "mta_date", "dt"],
)
